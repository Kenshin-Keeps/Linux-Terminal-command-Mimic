...
void printPriority(struct stat statinfo)      // Prints user/group access permisions
{
    ...
    cout << " ";
    cout << statinfo.st_nlink << " ";		// Prints number of hard links to the file
}
...
int getBlockSize(const char* directory) {
   ...
   if (d) {
       while ((dir = readdir(d)) != NULL) {
           if (dir->d_name[0] != '.') { 
               char info_path[PATH_MAX + 1];			// Create the path to stat
...
void newls(const char *dir, int op_a, int op_l, int op_t){
	...
	if(op_l){				
		size = getBlockSize(".");	//Finding total blocksize occipied by all the files
		cout << "total " << size;
		cout << "\n"; 
	}
